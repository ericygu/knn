...
def standard_scale(xTrain, xTest):
    ...
    """
    # TODO FILL IN
    #xTrainScaled = preprocessing.scale(np.array(xTrain))
    #xTestScaled = preprocessing.scale(np.array(xTest))
    #x = xTrain.values
    standard_scaler = StandardScaler()
    xTrainScaled = standard_scaler.fit_transform(xTrain)
    xTestScaled = standard_scaler.fit_transform(xTest)
    
    xTrainScaled_df = pd.DataFrame(xTrainScaled)#, index=xTrainScaled.index, columns=xTrainScaled.columns
    xTestScaled_df = pd.DataFrame(xTestScaled)#, index=xTrainScaled.index, columns=xTrainScaled.columns
    
    return xTrainScaled_df, xTestScaled_df
...
def minmax_range(xTrain, xTest):
    ...
    xTrainRanged = min_max_scaler.fit_transform(xTrain)
    xTestRanged = min_max_scaler.fit_transform(xTest)
    xTrainRanged_df = pd.DataFrame(xTrainRanged)
    xTestRanged_df = pd.DataFrame(xTestRanged)
    return xTrainRanged_df, xTestRanged_df
...
def add_irr_feature(xTrain, xTest):
    ...
    """
    # TODO FILL IN
    data1 = np.random.randn(xTrain.shape[0])
    data2 = np.random.randn(xTest.shape[0])
    
    xTrainG = xTrain.assign(Irr1 = data1)
    xTrainGauss = xTrainG.assign(Irr2 = data1)
    
    xTestG = xTest.assign(Irr1 = data2)
    xTestGauss = xTest.assign(Irr2 = data2)
    return xTrainGauss, xTestGauss
